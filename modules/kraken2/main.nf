...
process RUN_KRAKEN2_DECONTAM {
    tag "${sample_id}"
    cpus 8
    publishDir( params.output_dir ?: "results", mode: 'copy', overwrite: true, saveAs: { fn -> "${sample_id}/reads/kraken2/${file(fn).name}" } )
    ...
    set -euo pipefail
    mkdir -p kraken2
    kraken2 --db ${kraken2_db} --paired ${reads1} ${reads2} --threads ${threads} --use-names --report-zero-counts --report ${report} --output ${output}
