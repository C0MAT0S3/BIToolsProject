...
workflow RUN_ALL_TOOLS {
    ...
    def bwa_idx_dir_ch       = params.run_bwa        ? BUILD_BWA_INDEX(file(host_ref_fasta)).index_dir : Channel.empty()
    ...
    def bwa_out     = params.run_bwa        ? RUN_BWA_DECONTAM(reads_ch, bwa_idx_dir_ch) : [ cleaned: Channel.empty(), host: Channel.empty() ]
    ...
    def bowtie2_ids = bowtie2_out.host.join(bowtie2_out.cleaned, by:0).map { sid, h1, h2, c1, c2 ->
        tuple(sid, 'bowtie2', h1, h2, c1, c2)
    }
    def bwa_ids = bwa_out.host.join(bwa_out.cleaned, by:0).map { sid, h1, h2, c1, c2 ->
        tuple(sid, 'bwa', h1, h2, c1, c2)
    }
    def kraken2_ids = k2_out.host.join(k2_out.cleaned, by:0).map { sid, h1, h2, c1, c2 ->
        tuple(sid, 'kraken2', h1, h2, c1, c2)
    }
    def krakenuniq_ids = ku_out.host.join(ku_out.cleaned, by:0).map { sid, h1, h2, c1, c2 ->
        tuple(sid, 'krakenuniq', h1, h2, c1, c2)
    }
    def kneaddata_ids = kd_out.host.join(kd_out.cleaned, by:0).map { sid, h1, h2, c1, c2 ->
        tuple(sid, 'kneaddata', h1, h2, c1, c2)
    }
    def kmcp_ids = kmcp_out.host_ids.join(kmcp_out.cleaned, by:0).map { sid, ids, c1, c2 ->
        tuple(sid, 'kmcp', ids, ids, c1, c2)
    }
